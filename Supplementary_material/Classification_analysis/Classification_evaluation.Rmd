...
#### 5. Different sequence lengths
```{r echo=FALSE}
result <- fread("/Users/fengqian/Downloads/UniMelb_shared-master/algorithm_simulation/protein/snake_pipeline/final_results_seq_length/results_all/result.csv"
                            , header=TRUE
                          , data.table = FALSE)
result_sd <- fread("/Users/fengqian/Downloads/UniMelb_shared-master/algorithm_simulation/protein/snake_pipeline/final_results_seq_length/results_all/result_sd.csv"
                            , header=TRUE
                          , data.table = FALSE)
#se
sequence_length  <-seq(100,300,by=50)
once_only <- result$sensitivity[1:5];twice_only <- result$sensitivity[6:10]
df <- data.frame(sequence_length, once_only, twice_only)
df2 <- melt(data = df, id.vars = "sequence_length")# melt the data to a long format
once_only <- result_sd$sensitivity[1:5];twice_only <- result_sd$sensitivity[6:10]
df <- data.frame(sequence_length, once_only, twice_only)
df3 <- melt(data = df, id.vars = "sequence_length")# melt the data to a long format
df2$CI.L=df2$value-qnorm(.975)*df3$value/10
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
sensivity=ggplot(data = df2, aes(x = sequence_length, y = value,colour=variable)) + geom_line()+geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_continuous(breaks=seq(min(sequence_length),max(sequence_length),by=50))+scale_y_continuous(breaks=seq(0.65,0.95,by=0.05),limits=c(0.65,0.95))+ylab("Sensitivity")+xlab("Sequence length")+theme(legend.position = "none")
#sp
sequence_length  <-seq(100,300,by=50)
once_only <- result$specifity[1:5]
twice_only <- result$specifity[6:10]
...
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
sp <- ggplot(data = df2, aes(x = sequence_length, y = value,colour=variable)) + geom_line()+geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_continuous(breaks=seq(min(sequence_length),max(sequence_length),by=50))+scale_y_continuous(breaks=seq(0.75,0.95,by=0.02),limits=c(0.75,0.95))+ylab("Specifity")+xlab("Sequence length")+theme(legend.position = "none")
#sv
sequence_length  <-seq(100,300,by=50)
once_only_rec <- result$sv_rec[1:5];once_only_nonrec <- result$sv_nonrec[1:5];
twice_only_rec <- result$sv_rec[6:10];twice_only_nonrec <- result$sv_nonrec[6:10];
df <- data.frame(sequence_length, once_only_rec, once_only_nonrec,twice_only_rec,twice_only_nonrec)
df2 <- melt(data = df, id.vars = "sequence_length")# melt the data to a long format
once_only_rec <- result_sd$sv_rec[1:5];once_only_nonrec <- result_sd$sv_nonrec[1:5];
twice_only_rec <- result_sd$sv_rec[6:10];twice_only_nonrec <- result_sd$sv_nonrec[6:10];
df <- data.frame(sequence_length, once_only_rec, once_only_nonrec,twice_only_rec,twice_only_nonrec)
...
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
sv <- ggplot(data = df2, aes(x = sequence_length, y = value,colour=variable)) + geom_line()+geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_continuous(breaks=seq(min(sequence_length),max(sequence_length),by=50))+scale_y_continuous(breaks=seq(0.66,0.90,by=0.02),limits=c(0.66,0.90))+ylab("Support value mean")+xlab("Sequence length")
#rec
sequence_length  <-seq(100,300,by=50)
once_only <- result$rho[1:5]
twice_only <- result$rho[6:10]
...
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
rec=ggplot(data = df2, aes(x = sequence_length, y = value,colour=variable)) + geom_line()+geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_continuous(breaks=seq(min(sequence_length),max(sequence_length),by=50))+scale_y_continuous(breaks=seq(0.002,0.012,by=0.002),limits=c(0.002,0.012))+ylab("Estimated recombination rate")+xlab("Sequence length")
sv
```
...
#### 6. Different amino acid models.
```{r echo=FALSE}
result <- fread("/Users/fengqian/Downloads/UniMelb_shared-master/algorithm_simulation/protein/snake_pipeline/final_results_AA_models/results_all/result.csv"
                            , header=TRUE
                          , data.table = FALSE)
result_sd <- fread("/Users/fengqian/Downloads/UniMelb_shared-master/algorithm_simulation/protein/snake_pipeline/final_results_AA_models/results_all/result_sd.csv"
                            , header=TRUE
                          , data.table = FALSE)
#se
mutation_rate  <-seq(1,6,by=1)
once_only <- result$sensitivity[1:6];twice_only <- result$sensitivity[7:12]
df <- data.frame(mutation_rate, once_only, twice_only)
df2 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
once_only <- result_sd$sensitivity[1:6];twice_only <- result_sd$sensitivity[7:12]
df <- data.frame(mutation_rate, once_only, twice_only)
df3 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
df2$CI.L=df2$value-qnorm(.975)*df3$value/10
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
sensivity=ggplot(data = df2, aes(x = factor(mutation_rate), y = value,colour=variable))+geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_discrete(breaks=c("1","2","3","4","5","6"),labels=c("AB", "DAYHOFF", "JTT","LG","MTMAM","WAG"))+scale_y_continuous(breaks=seq(0.70,0.95,by=0.02),limits=c(0.70,0.95))+ylab("Sensitivity")+xlab("Empirical model")+theme(legend.position = "none")
#sp
mutation_rate  <-seq(1,6,by=1)
once_only <- result$specifity[1:6]
twice_only <- result$specifity[7:12]
df <- data.frame(mutation_rate, once_only, twice_only)
df2 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
once_only <- result_sd$specifity[1:6]
twice_only <- result_sd$specifity[7:12]
df <- data.frame(mutation_rate, once_only, twice_only)
df3 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
df2$CI.L=df2$value-qnorm(.975)*df3$value/10
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
sp <- ggplot(data = df2, aes(x = factor(mutation_rate), y = value,colour=variable)) +geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_discrete(breaks=c("1","2","3","4","5","6"),labels=c("AB", "DAYHOFF", "JTT","LG","MTMAM","WAG"))+scale_y_continuous(breaks=seq(0.75,0.95,by=0.02),limits=c(0.75,0.95))+ylab("Specifity")+xlab("Empirical model")+theme(legend.position = "none")
#sv
mutation_rate  <-seq(1,6,by=1)
once_only_rec <- result$sv_rec[1:6];once_only_nonrec <- result$sv_nonrec[1:6];
twice_only_rec <- result$sv_rec[7:12];twice_only_nonrec <- result$sv_nonrec[7:12];
df <- data.frame(mutation_rate, once_only_rec, once_only_nonrec,twice_only_rec,twice_only_nonrec)
df2 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
once_only_rec <- result_sd$sv_rec[1:6];once_only_nonrec <- result_sd$sv_nonrec[1:6];
twice_only_rec <- result_sd$sv_rec[7:12];twice_only_nonrec <- result_sd$sv_nonrec[7:12];
df <- data.frame(mutation_rate, once_only_rec, once_only_nonrec,twice_only_rec,twice_only_nonrec)
df3 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
df2$CI.L=df2$value-qnorm(.975)*df3$value/10
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
sv <- ggplot(data = df2, aes(x = factor(mutation_rate), y = value,colour=variable)) +geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_discrete(breaks=c("1","2","3","4","5","6"),labels=c("AB", "DAYHOFF", "JTT","LG","MTMAM","WAG"))+scale_y_continuous(breaks=seq(0.65,0.90,by=0.02),limits=c(0.65,0.90))+ylab("Support value mean")+xlab("Empirical model")
#rec
mutation_rate  <-seq(1,6,by=1)
once_only <- result$rho[1:6]
twice_only <- result$rho[7:12]
df <- data.frame(mutation_rate, once_only, twice_only)
df2 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
once_only <- result_sd$rho[1:6]
twice_only <- result_sd$rho[7:12]
df <- data.frame(mutation_rate, once_only, twice_only)
df3 <- melt(data = df, id.vars = "mutation_rate")# melt the data to a long format
df2$CI.L=df2$value-qnorm(.975)*df3$value/10
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
rec=ggplot(data = df2, aes(x = factor(mutation_rate), y = value,colour=variable)) +geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_discrete(breaks=c("1","2","3","4","5","6"),labels=c("AB", "DAYHOFF", "JTT","LG","MTMAM","WAG"))+scale_y_continuous(breaks=seq(0.002,0.01,by=0.002),limits=c(0.002,0.01))+ylab("Estimated recombination rate")+xlab("Empirical model")
sv
```
