...
df2$CI.R=df2$value+qnorm(.975)*df3$value/10
# plot, using the aesthetics argument 'colour'
rec=ggplot(data = df2, aes(x = mutation_rate, y = value,colour=variable)) + geom_line()+geom_point()+geom_errorbar(width=.001, aes(ymin=CI.L, ymax=CI.R))+scale_x_continuous(breaks=c(1,2,3,4,5,6,7,8,9),labels=c("0.2","0.4", "0.6","0.8","1","2","3","4","5"))+scale_y_continuous(breaks=seq(0.002,0.01,by=0.002),limits=c(0.002,0.01))+ylab(expression("Estimated"~rho))+xlab("Branch length multiplier")
sv
```
