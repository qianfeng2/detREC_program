...
density_rec=ggplot(rec_pro_50, aes(x=bkp)) + geom_density(adjust = 5)+geom_histogram(aes(y=..density..),colour="black", fill="gray")+scale_x_continuous(breaks=seq(-30,30,by=5),limits=c(-30,30))+xlab("Difference of breakpoint locations")+ylab("Density")+theme(axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))

density_2d_rec=ggplot(simulation_50_50_output, aes(x = identified_bkp, y = actual_bkp) ) +geom_bin2d()+xlab("Identified breakpoint location")+ylab("True breakpoint location")+theme(axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))+scale_x_continuous(expand = c(0, 0),breaks=seq(0,200,by=50),limits=c(0,200))+scale_y_continuous(expand = c(0, 0))
ggarrange(density_2d_rec, density_rec, ncol = 2, labels = c("A", "B"))
dev.off()
```
