...
```{r}
apply(simulation_70_30_output$bkp_error,mean)
```
...
#ggplot(rec_pro) + geom_density(aes(x = bkp), bw = 1) + facet_wrap(~name)+scale_x_continuous(breaks=seq(-30,30,by=5),limits=c(-30,30))+     geom_rug(data=rec_pro_sample, aes(x = bkp, y = 0), position = position_jitter(height = 0), inherit.aes = F)
...
pdf(file="/Users/fengqian/Downloads/rec_paper_version3/figures/bkp.pdf",width=12,height=8,onefile=F)
density_rec=ggplot(rec_pro_50) + geom_density(aes(x = bkp),adjust=2)+scale_x_continuous(breaks=seq(-30,30,by=5),limits=c(-30,30))+geom_rug( aes(x = bkp, y = 0), position = position_jitter(height = 0))+xlab("Difference of breakpoint locations")+ylab("Density")+theme(axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))
scatter_rec=ggplot(simulation_50_50_output, aes(x = identified_bkp, y = actual_bkp)) +
    geom_point()+xlab("Identified breakpoint location")+ylab("True breakpoint location")+theme(axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))+scale_x_continuous(expand = c(0, 0),breaks=seq(0,200,by=50),limits=c(0,200))+scale_y_continuous(expand = c(0, 0))
ggarrange(scatter_rec, density_rec, ncol = 2, labels = c("A", "B"))
dev.off()
```
